...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/13 13:37:34 by dpylypen          #+#    #+#             */
/*   Updated: 2017/04/13 13:41:15 by dpylypen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	iter(t_scene *scene, size_t *i)
{
	...
	{
		if (fabs(scene->z_im * scene->z_im +
	scene->z_re * scene->z_re) > 4)
			break ;
		scene->z_im1 = 2 * fabs(scene->z_re * scene->z_im) + scene->c_im;
		scene->z_re = scene->z_re * scene->z_re - scene->z_im * scene->z_im
	+ scene->c_re;
		scene->z_im = scene->z_im1;
	}
}
...
void		burning_ship(t_scene *scene)
{
	...
	while (++y < scene->height_thr)
	{
		...
		while (++x < scene->w)
		{
			...
			scene->z_re = 0 + scene->change_re;
			scene->z_im = 0 + scene->change_im;
			iter(scene, &i);
			color(scene, i, x, y);
		}
	}
