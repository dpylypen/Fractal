...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/13 13:50:04 by dpylypen          #+#    #+#             */
/*   Updated: 2017/04/13 13:52:18 by dpylypen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	iter(t_scene *scene, size_t *i)
{
	...
	while (++(*i) < scene->max_iter)
	{
		if (fabs(scene->z_im * scene->z_im + scene->z_re * scene->z_re) > 4)
			break ;
		scene->z_re1 = scene->z_re;
		scene->z_im1 = scene->z_im;
...
void		julia(t_scene *scene)
{
	...
	scene->w + scene->min_re + scene->move_x;
			scene->z_im = y * (scene->max_re - scene->min_re) /
	scene->h + scene->min_im + scene->move_y;
			iter(scene, &i);
			if (scene->col_mod == 0)
